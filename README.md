...
| 選択 | 動作 |
|---|---|
| 1〜3（今週 / 来週 / 再来週） | その週の月〜金を1日ずつ登録する（ランチャー側で組み立てる） |
| 4（その他の操作） | `backlog_issue_cloner/menu.py` を起動する（単純複製・直接更新） |

**週次一括だけはランチャー側に残しています。** ツール側のメニューは1日ぶんしか
扱わないため、委譲すると月〜金の一括登録ができなくなるためです。
委譲の選択肢を末尾に置いているのは、`menu.py 3 2 1` のような既存の無人実行
コマンドの番号を変えないためです。

日付のルール: 選択した週の月〜金を1日ずつ処理します。**今週を選んだ場合、当日以前は
スキップ**され、翌日以降の日付だけが登録対象になります。来週・再来週は5日分すべてが
対象です。
